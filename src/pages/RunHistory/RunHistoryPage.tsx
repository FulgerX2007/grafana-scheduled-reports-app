    ...
    try {
      ...
      if (scheduleId) {
        ...
        response = await getBackendSrv().get(
          `/api/plugins/fulgerx2007-scheduledreports-app/resources/api/schedules/${scheduleId}/runs`
        );
        setRuns(response.runs || []);
      } else {
        // Load all runs from all schedules
        const schedulesResponse = await getBackendSrv().get('/api/plugins/fulgerx2007-scheduledreports-app/resources/api/schedules');
