...
export const SettingsPage: React.FC<SettingsPageProps> = ({ onNavigate }) => {
  ...
  const loadSettings = async () => {
    try {
      const response = await getBackendSrv().get('/api/plugins/scheduledreports-app/resources/api/settings');
      if (response) {
  ...
  const loadServiceAccountStatus = async () => {
    try {
      const response = await getBackendSrv().get('/api/plugins/scheduledreports-app/resources/api/service-account/status');
  ...
  const handleSubmit = async () => {
    ...
    try {
      await getBackendSrv().post('/api/plugins/scheduledreports-app/resources/api/settings', settings);
      appEvents.publish({
  ...
  const handleCheckChromium = async () => {
    ...
    try {
      const response = await getBackendSrv().post(
        '/api/plugins/scheduledreports-app/resources/api/chromium/check-version',
  ...
  const handleTestSMTP = async () => {
    ...
    try {
      const response = await getBackendSrv().post(
        '/api/plugins/scheduledreports-app/resources/api/smtp/test',
        settings.smtp_config
      );
